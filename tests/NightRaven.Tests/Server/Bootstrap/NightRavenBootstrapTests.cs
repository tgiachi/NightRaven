...
public sealed class NightRavenBootstrapTests : IDisposable
{
    ...
    {
        var options = new NightRavenBootstrapOptions([], _root, debug: false, showHeader: false);
    ...
    {
        var args = new[] { "--debug" };
        var options = new NightRavenBootstrapOptions(args, _root, debug: true, showHeader: false);
    ...
    public void ConfigureContainer_RegistersCoreServicesAndConfig()
    {
        ...
        var context = NightRavenBootstrap.CreateContext(new([], _root, debug: false, showHeader: false));
