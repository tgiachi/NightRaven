...
public class SpanWriterTests
{
    ...
    public void WriteAscii_FixedLengthPadsWithNulls()
    {
        Span<byte> backing = stackalloc byte[8];
        var writer = new SpanWriter(backing);
        writer.WriteAscii("hi", fixedLength: 5);
    ...
    public void WriteAscii_FixedLengthTruncatesLongString()
    {
        Span<byte> backing = stackalloc byte[8];
        var writer = new SpanWriter(backing);
        writer.WriteAscii("hello world", fixedLength: 5);
    ...
    public void WriteLittleUni_FixedLengthPadsWithUnicodeNulls()
    {
        Span<byte> backing = stackalloc byte[16];
        var writer = new SpanWriter(backing);
        writer.WriteLittleUni("hi", fixedLength: 5);
    ...
    {
        var writer = new SpanWriter(initialCapacity: 4, resize: true);
    ...
    {
        var writer = new SpanWriter(initialCapacity: 8, resize: true);
        writer.Write((byte)1);
        writer.Write((byte)2);
        writer.Write((byte)3);
    ...
    {
        var writer = new SpanWriter(initialCapacity: 64, resize: true);
